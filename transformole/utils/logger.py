...
class MoleculeGenerationCallback(pl.Callback):
    def on_epoch_end(self, trainer, pl_module):
        ...
        csv_path = pl_module.generate(num_samples=100, max_length=100, output_dir=output_dir, vocab_path='path/to/vocab.yaml')

        # Calculate metrics
        avg_atom_count = calculate_atom_count_distribution(csv_path)
        valid_ratio = calculate_valid_smiles_ratio(csv_path)
        avg_max_similarity = calculate_similarity(csv_path, 'path/to/target.csv')
